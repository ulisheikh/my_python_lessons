...
from language_packages import lang_pack


console = Console()

for til,text in lang_pack.items():
    print(f"\nTil: {til}")
    n = 1
...
def sontop_PC(x):
    ...
    while flag:
        ...
        if javob.isdigit():
            ...
            javob = int(javob)
            if javob < son:
                console.print(f"Men o'ylagan son {javob} dan katta",style='bold yellow')
                taxmin_user += 1
            elif javob > son:
                console.print(f"Men o'ylagan son {javob} dan kichik",style='bold yellow')
                taxmin_user += 1
            elif javob == son:
                console.print("Siz topdingiz",style='rgb(0,255,0) bold')
                taxmin_user += 1
                flag = 0
